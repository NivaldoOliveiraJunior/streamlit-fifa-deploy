...
if "data" not in st.session_state:
    ...
    # validação para trazer apenas os dados que queremos
    df_data = df_data[df_data["Contract Valid Until"] >= datetime.today().year]
    df_data = df_data[df_data["Value(£)"] > 0]
    # agora estamos ordenando por overwall
    df_data = df_data.sort_values(by="Overall", ascending=False)
    st.session_state["data"] = df_data

st.markdown("# FIFA 2023 OFFICIAL!")
st.sidebar.markdown("Desenvolvido por [Brululu](https://www.brululu.com.br)")
...
if btn:
    webbrowser.open_new_tab("https://www.kaggle.com/datasets/bryanb/fifa-player-stats-database?select=FIFA23_official_data.csv")